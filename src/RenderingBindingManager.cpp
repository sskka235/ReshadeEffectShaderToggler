...
RenderingBindingManager::RenderingBindingManager(AddonImGui::AddonUIData& data, ResourceManager& rManager, ToggleGroupResourceManager& tgResources)
  : uiData(data)
  , resourceManager(rManager)
  , toggleGroupResources(tgResources) {}
...
void RenderingBindingManager::InitTextureBingings(effect_runtime* runtime) {
    ...
    CreateTextureBinding(runtime,
                         &data.bindingManagerData.empty_res,
                         &data.bindingManagerData.empty_srv,
                         &data.bindingManagerData.empty_rtv,
...
bool RenderingBindingManager::_CreateTextureBinding(reshade::api::effect_runtime* runtime,
    ...
    if (*srv == 0 &&
        !runtime->get_device()->create_resource_view(*res, resource_usage::shader_resource, resource_view_desc(format_to_default_typed(format, 0)), srv)) {
        reshade::log::message(reshade::log::level::error, "Failed to create texture binding resource view!");
        return false;
    }

    if (*rtv == 0 &&
        !runtime->get_device()->create_resource_view(*res, resource_usage::render_target, resource_view_desc(format_to_default_typed(format, 0)), rtv)) {
        reshade::log::message(reshade::log::level::error, "Failed to create texture binding resource view!");
        return false;
    }
...
bool RenderingBindingManager::CreateTextureBinding(effect_runtime* runtime,
                                                   resource* res,
                                                   resource_view* srv,
                                                   resource_view* rtv,
                                                   reshade::api::format format) {
...
uint32_t RenderingBindingManager::UpdateTextureBinding(effect_runtime* runtime,
                                                       ToggleGroup* group,
                                                       resource res,
    ...
    if (groupResource.state == ShaderToggler::GroupResourceState::RESOURCE_RECREATED ||
        groupResource.state == ShaderToggler::GroupResourceState::RESOURCE_CLEARED) {
...
void RenderingBindingManager::_QueueOrDequeue(command_list* cmd_list,
                ...
                if (retUpdate && target_res != 0) {
                    ...
                    if (group->getFlipBufferBinding() && bindingResource.rtv != 0 && runtimeData.specialEffects[REST_FLIP].technique != 0) {
                        deviceData.current_runtime->render_technique(
                          runtimeData.specialEffects[REST_FLIP].technique, cmd_list, bindingResource.rtv, bindingResource.rtv_srgb);
                    }
                }
            }
...
void RenderingBindingManager::UpdateTextureBindings(command_list* cmd_list, uint64_t callLocation, uint64_t invocation) {
    ...
    if (deviceData.current_runtime == nullptr || (commandListData.ps.bindingsToUpdate.size() == 0 && commandListData.vs.bindingsToUpdate.size() == 0 &&
                                                  commandListData.cs.bindingsToUpdate.size() == 0)) {
        return;
    }
...
void RenderingBindingManager::ClearUnmatchedTextureBindings(reshade::api::command_list* cmd_list) {
    ...
    for (auto& groupData : uiData.GetToggleGroups()) {
        ...
        if (!data.bindingsUpdated.contains(&group) &&
            (resources.clear_on_miss() && data.bindingManagerData.empty_srv != 0 && resources.state != ShaderToggler::GroupResourceState::RESOURCE_CLEARED)) {
            data.current_runtime->update_texture_bindings(
              group.getTextureBindingName().c_str(), data.bindingManagerData.empty_srv, data.bindingManagerData.empty_srv);
